...
[<CLIMutable>]
type Person = { ID: int; LastName: string; FirstName: string }
...
let typedInsertSql = "INSERT INTO People (LastName, FirstName) VALUES (@lastName, @firstName)"

conn
  .Execute(
    typedInsertSql,
    { LastName = Guid.NewGuid().ToString(); FirstName = Guid.NewGuid().ToString() })
|> ignore
